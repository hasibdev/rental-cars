  ...
  <div class="b-files">
    ...
    <ul v-if="files" class="file-uploader__selected-files">
      <li v-for="(file, index) in files" :key="index">
        ...
        <button @click="removeFile(index, file.id)" class="file-uploader__rm-btn">
...
export default {
  ...
  methods: {
    ...
    async removeFile(index, id) {
      try {
        await http.delete(`${this.rmEndpoint}/${id}`)
    ...
    async sendFile() {
      ...
      try {
        const data = await http.post(this.endpoint, formData, {
          onUploadProgress: (e) =>
            (this.progress = Math.round((e.loaded * 100) / e.total)),
        })
        this.files = data
        const fileValue = this.files.map((_) => _[this.valueProperty])
        this.$emit("update:modelValue", fileValue)
      } catch (err) {
        console.error(err)
      }
    },
    async removePreviewsFiles() {
      try {
        await http.delete(`${this.rmAllEndpoint}/${this.formName}`)
        this.files = []
      } catch (err) {
        console.error(err)
      }
