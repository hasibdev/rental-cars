...
const onFocus = () => {
  if (!props.modelValue || props.modelValue == 0) {
    ...
    const num = Number(props.modelValue).toLocaleString('de-DE', { maximumFractionDigits: 2 })
    emit('update:currency', num)
  }
}
  ...
  <div class="row align-items-center">
    ...
    <div :class="`${(props.label && !props.stack) ? 'col-xl-8' : 'col-md-12'}`">
      <div class="relative">
        <input @input="onChange" @blur="onBlur" @focus="onFocus" :value="props.currency" :type="props.type" :readonly="props.readonly" class="form-control" :class="{'border-danger has_error': getError || hasError}" :disabled="props.disabled" :placeholder="placeholder">
