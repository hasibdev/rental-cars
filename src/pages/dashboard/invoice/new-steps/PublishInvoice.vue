...
const { invoiceStage } = useInvoiceStore()
const toast = useToast()
const router = useRouter()
// const route = useRoute()

const { data: invoiceData, loading, reFetch } = useAxios(`admin/invoice/view/${invoiceStage.createPreviewId}`)
...
const sendEmail = async () => {
  try {
    ...
    await api.post(`admin/invoice/invoice-mail/${invoiceStage.createPreviewId}`, formData)
...
const fetchPDF = async () => {
  try {
    const res = await api.get(`admin/invoice/pdf/${invoiceStage.createPreviewId}`, {
      responseType: 'blob'
    })
...
<template>
  <div v-if="!loading">
    <!-- <h5 class="d-flex align-items-center mb-3 ">
      <span class="ms-3"> {{t('dashboard-payment.payment')}} </span>
    </h5> -->
