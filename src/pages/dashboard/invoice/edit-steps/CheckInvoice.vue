  ...
  <div class="d-flex justify-content-center flex-wrap w-100 scrollbar">
    <div class="mb-4" style="width: 596px;">
      <div class="borderd-content" style="border-radius: 4px 4px 0px 0px; border-bottom: none; ">
        ...
        <h3 class="mt-5 mb-4">{{ t('form-fields.invoice') }} {{edit.invoice_no}}</h3>

        <p class="text-12 fw-400">{{ edit.introduction_text }}</p>
        ...
        <table class="table table-borderless rounded overflow-hidden">
          ...
          <tbody>
            <tr v-for="(line, ii) in edit.lines" :key="ii" :style="{background: ii % 2 == 0 ? '#f6f7fb' : '#eee'}">
              ...
              <td class="p-4">{{ moneyFormat(line.rate) }}</td>
              <td class="p-4">{{ moneyFormat(line.amount) }}</td>
            </tr>
          </tbody>
        </table>
        ...
        <div class="total-summary my-4">
          <div class="row mb-2">
            ...
            <div class="col-md-2">
              <div class="text-12 fw-500 text-right">{{ moneyFormat(edit.sub_total) }}</div>
            </div>
          </div>
          <div class="row pb-1 mb-2">
            ...
            <div class="col-md-2">
              <div class="text-12 fw-500 text-right">{{moneyFormat(edit.discount_amount)}}</div>
            </div>
          </div>
          <div class="row pb-1 mb-4">
            <div class="col-md-10">
              <div class="text-12 fw-400 text-right" style="color: rgba(0,0,0,0.6);">{{t('form-fields.tax')}}({{ edit.tax }}%)</div>
            </div>
            <div class="col-md-2">
              <div class="text-12 fw-500 text-right">{{ moneyFormat(edit.tax_amount) }}</div>
            </div>
          </div>
          <div class="d-flex justify-content-end">
            <div class="bg-dark text-white d-flex align-items-center justify-content-between" style="border-radius: 4px; padding: 10px 22px;">
              ...
              <span class="text-22 fw-700">{{ moneyFormat(edit.grand_total) }}</span>
            </div>
          </div>
        </div>
        <div class="pb-4">
        ...
        <div v-if="edit.termas_conditions" class="w-50 dashed-text-field mb-4">
          <div class="text-12 fw-700" style="color: rgba(0,0,0,0.6);">{{ t('dashboard-invoice.terms-and-conditions') }}</div>
          <p class="text-10 fw-400" style="color: rgba(0, 0, 0, 0.5);">{{edit.termas_conditions}}</p>
        </div>

        <p class="text-12 fw-400">{{ edit.final_text }}</p>
    ...
    <div class="mb-4 px-4" style="width: 350px; margin-left: 60px;">
      <PrimaryButton @click="emits('changeTab', 1)" variant="light" class="w-100">
        <svg class="me-2" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
          <g clip-path="url(#clip0_4203_5231)">
            <path d="M11.332 1.9976C11.5071 1.8225 11.715 1.68361 11.9438 1.58885C12.1725 1.49409 12.4177 1.44531 12.6654 1.44531C12.913 1.44531 13.1582 1.49409 13.387 1.58885C13.6157 1.68361 13.8236 1.8225 13.9987 1.9976C14.1738 2.17269 14.3127 2.38056 14.4074 2.60934C14.5022 2.83811 14.551 3.08331 14.551 3.33093C14.551 3.57855 14.5022 3.82375 14.4074 4.05253C14.3127 4.2813 14.1738 4.48917 13.9987 4.66426L4.9987 13.6643L1.33203 14.6643L2.33203 10.9976L11.332 1.9976Z" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
          </g>
          <defs>
      ...
      <PrimaryButton @click="saveInvoice" :loading="savingStatus" class="w-100 mt-4">
        ...
        {{ t('dashboard-invoice.update-invoice') }}
      </PrimaryButton>
    </div>
