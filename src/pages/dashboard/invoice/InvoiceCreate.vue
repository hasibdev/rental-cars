<script setup>
import { ref, markRaw, watch, onUnmounted } from 'vue'
import { useI18n } from 'vue-i18n'
import BackButton from "@/components/buttons/BackButton.vue"
import CreateInvoice from './new-steps/CreateInvoice.vue'
import CheckInvoice from './new-steps/CheckInvoice.vue'
import PublishInvoice from './new-steps/PublishInvoice.vue'
...
const { form, clearForm, invoiceStage } = useInvoiceStore()
...
const steps = ref([
  {
    id: 1,
    name: t('dashboard-invoice.create-invoice'),
    backLabel: t('dashboard-invoice.create-invoice'),
    component: markRaw(CreateInvoice)
  },
  {
    id: 2,
...
watch(() => generalInfo.value, (val) => {
  form.currency = val.store_currency
})
...
const saveInvoice = async () => {
  try {
    ...
    const res = await api.post('admin/invoice/store', form)
    invoiceStage.createPreviewId = res.data.invoice_id
