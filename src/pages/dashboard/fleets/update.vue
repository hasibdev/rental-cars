...
// import ImageUploader from '@/components/dashboard/form/ImageUploader.vue'
...
const submitForm = async () => {
  try {
    ...
    formData.append(`checks`, JSON.stringify(form.checks))

    await api.post(`/admin/vehicle/update/${route.params.id}`, formData)
    documents.value = []
...
const fetchFleet = async () => {
  const res = await api.get(`admin/vehicle/edit/${route.params.id}`)
  Object.assign(form, res.data.datainfo)

  form.id = `${route.params.id}`
  const checks = JSON.parse(res.data.datainfo.checks)
  if (checks.length) {
    form.checks = checks
  } else {
    form.checks = [{
  ...
  previewDocuments.value = res.data.documents.map(doc => ({
    img: imageBaseUrl(`/${doc.folder}/${doc.file_name}`),
    id: doc.id,
    name: doc.file_name
  }))
}
...
const removePrevDoc = id => {
  previewDocuments.value = previewDocuments.value.filter(doc => doc.id != id)
}

// Art Count
const addToArtCount = (count) => {
  form.art_count = count
}
...
<template>
  ...
  <div class="fleet-form">
    <div class="d-flex align-items-center mb-4">
      <BackButton /> <span class="add-fleet__title ms-3">{{t('dashboard-fleet.edit-fleet')}}</span>
    </div>
    <div class="fleet-form">
      <div class="edit-fleet__container">
        <div class="row">
          <div class="col-lg-6 col-md-6 col-sm-12">
            <InputControl v-model="form.id" label="ID" errorField="vehicle_name" class="mt-3" disabled />
            ...
            <SelectControl v-model="form.class_id" optionLabel="classname" :options="fleetClasseData || [] " :placeholder="t('FleetCreate.select-fleet-class')" :label="t('FleetCreate.select-fleet-class')" errorField="classname" class="mt-3" />
            ...
            <!-- <InputControl v-model="form.fuel_lable" :label="t('FleetCreate.fuel-level')" placeholder="Eg: 100%" errorField="fuel_lable" class="mt-3" /> -->
            ...
            <div v-for="(check, idx) in form.checks" :key="idx" class="check__item">
              <InputControl v-model="check.check" :label="t('FleetCreate.checks')" :placeholder="t('FleetCreate.checks')" class="mt-3" />
              <CalenderControl v-model="check.check_date" :label="$t('FleetCreate.check-date')" placeholder="DD/MM/YYYY" class="mt-3" />
              <svg v-if="form.checks.length !== 1" @click="removeCheck(idx)" class="remove_icon" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <div class="col-lg-5 col-md-6 offset-lg-1 col-sm-12">
            <div class="row mx-0 mb-4">
              ...
              <div class="col-md-8 col-sm-12">
                <DocumentsCameraUploader :previousDocuments="previewDocuments" prevTrack="name" v-model="documents" @removePrev="removePrevDoc"></DocumentsCameraUploader>
              </div>
            </div>
            <div class="mb-4">
              <ArtBoard v-model="form.art" v-model:artCount="form.art_count" @artCount="addToArtCount" errorField="art" />
            </div>
            ...
            <div class="fleet-form__actions fleets-update-m mt-4">
              <button :loading="false" class="btn btn-cancel me-3" @click="cancelToCreate">{{t('button.cancel')}}
              </button>
              <!-- <BButton style="width: 150px" :loading="false" theme="secondary" @click="submitForm">{{ loading ? "Saving..." : "Save Fleet" }}</BButton> -->
              <PrimaryButton variant="dark" style="min-width: 180px;" @click="submitForm" :loading="savingState">
                {{t('dashboard-fleet.update-fleet')}}</PrimaryButton>
            </div>
          </div>
        </div>
      </div>
