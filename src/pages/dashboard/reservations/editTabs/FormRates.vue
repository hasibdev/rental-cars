...
const getRentalPeriod = computed(() => {
  // let days = parseInt(reserveStore.edit.rental_hours / 24)
  // let hours = parseInt(reserveStore.edit.rental_hours % 24)
  ...
  if (rd >= 30) {
    ...
    return `${mm} ${mm > 1 ? 'months' : 'month'}${dd > 0 ? `, ${dd} ${dd > 1 ? 'days' : 'day'}` : ``}`
  }
  ...
  if (rd >= 7) {
    ...
    return `${week} ${week > 1 ? 'weeks' : 'week'}${dd > 0 ? `, ${dd} ${dd > 1 ? 'days' : 'day'}` : ``}`
  }

  // Day
  // ${hours > 0 ? `, ${hours} hour` : ''
  return `${rd} ${rd > 1 ? `days` : `day`}`
})
...
const setTotalRentalPrice = () => {
  ...
  if (rentOption === 'day_rate') {
    ...
    reserveStore.edit.total_rental_price = total
    reserveStore.editCurrency.total_rental_price = deCurrency(total)
  }
...
const onRentOptionChange = val => {
  const rate = reserveStore.editInfo[val]
  reserveStore.edit.per_hour_rate = rate
  reserveStore.editCurrency.per_hour_rate = deCurrency(rate)
...
const getTotalWithuotVat = computed(() => {
  const total = Number(
    + Number(reserveStore.edit.total_rental_price)
    + Number(reserveStore.edit.pick_up_fee)
    + Number(reserveStore.edit.out_of_hours_fee)
    + Number(reserveStore.edit.others_costs)
    + Number(reserveStore.edit.charges_for_extras)
  )

  const d = Number(reserveStore.edit.discount)
  const discount = reserveStore.edit.discount_type == 'Round' ? d : (total / 100) * d
...
watch(() => getVatAmount.value, (newVal) => {
  reserveStore.edit.vat_amount = newVal
})

// Set Total Without val
watch(() => getTotalWithuotVat.value, (newVal) => {
  reserveStore.edit.total_without_vat = newVal
})

// Set SubTotal Without val
watch(() => getSubTotalWithuotVat.value, (newVal) => {
  reserveStore.edit.subtotal = newVal
})

// Set Total Discount Without val
watch(() => getDiscountWithuotVat.value, (newVal) => {
  reserveStore.edit.discount_amount = newVal
})


// Set to_pay in global form
watch(() => getToPayAmount.value, (newVal) => {
  reserveStore.edit.to_pay = newVal
})
...
<template>
  <div class="row">
    <div class="col-md-6 col-xxl-5">
      <RadioControl v-model="reserveStore.edit.calculate_value" name="calculate_value" @update:modelValue="onCalculateValueChange" :options="calculateOptions" :label="t('form-fields.calculation')" labelClasses="label-muted" class="mb-3" />
        ...
        <div class="col-md-8">
          <div class="row mb-3">
            <div class="col-5 col-xxl-4 pe-2">
              <CurrencyControl v-model.number="reserveStore.edit.per_hour_rate" v-model:currency="reserveStore.editCurrency.per_hour_rate" :disabled="reserveStore.edit.calculate_value == 'Automatic'" errorField="per_hour_rate" />
              <!-- <InputControl v-model.number="reserveStore.edit.per_hour_rate" type="number" :disabled="reserveStore.edit.calculate_value == 'Automatic'" appendIcon="€" /> -->
            </div>
            <div class="col-7 col-xxl-8 ps-2">
              <FormSelect v-model="reserveStore.edit.rent_option" @update:modelValue="onRentOptionChange" :options="rentForOptions" displayProperty="label" valueProperty="value" />
            </div>
          </div>
        </div>
      </div>

      <CurrencyControl :disabled="reserveStore.edit.calculate_value == 'Automatic'" v-model.number="reserveStore.edit.total_rental_price" v-model:currency="reserveStore.editCurrency.total_rental_price" :placeholder="`0.00`" :label="t('form-fields.total-rental-price') " errorField="per_hour_rate" :errors="props.validation" class="mb-3" appendIcon="€" />
      ...
      <CurrencyControl v-model.number="reserveStore.edit.pick_up_fee" v-model:currency="reserveStore.editCurrency.pick_up_fee" :placeholder="t('form-fields.pick-up-fee')" :label="t('form-fields.pick-up-fee')" errorField="pick_up_fee" :errors="props.validation" class="mb-3" />
      <CurrencyControl v-model.number="reserveStore.edit.out_of_hours_fee" v-model:currency="reserveStore.editCurrency.out_of_hours_fee" :placeholder="t('form-fields.out-of-hours-fee')" :label="t('form-fields.out-of-hours-fee')" errorField="out_of_hours_fee" :errors="props.validation" class="mb-3" />
      <CurrencyControl v-model.number="reserveStore.edit.others_costs" v-model:currency="reserveStore.editCurrency.others_costs" :placeholder="t('form-fields.other-costs')" :label="t('form-fields.other-costs')" errorField="others_costs" :errors="props.validation" class="mb-3 d-none" />
      <CurrencyControl v-model.number="reserveStore.edit.charges_for_extras" v-model:currency="reserveStore.editCurrency.charges_for_extras" :placeholder="t('form-fields.charges-for-extras')" :label="t('form-fields.charges-for-extras')" errorField="charges_for_extras" :errors="props.validation" class="mb-3" />

      <!-- <div class="row pt-1">
        <div class="col-md-4"></div>
        <div class="col-md-8">
          <RadioControl v-model="reserveStore.edit.discount_type" @update:modelValue="onChangeDiscountType" :options="discountTypes" />
        </div>
      </div> -->
      <InputControl v-model.number="reserveStore.edit.discount" type="number" :placeholder="t('form-fields.discount')" :label="t('form-fields.discount')" errorField="discount" :errors="props.validation" class="mb-3" :appendIcon="discountIcon" />
      <InputControl disabled :modelValue="deCurrency(getTotalWithuotVat)" :placeholder="t('form-fields.total-without-vat')" :label="t('form-fields.total-without-vat')" errorField="total_without_vat" :errors="props.validation" class="mb-3 d-none" />
      <InputControl disabled :modelValue="deCurrency(getVatAmount)" :placeholder="`${t('form-fields.vat')} (${taxRate}%)`" :label="`${t('form-fields.vat')} (${taxRate}%)`" errorField="vat_amount" :errors="props.validation" class="mb-3 d-none" />
      <InputControl disabled :modelValue="deCurrency(reserveStore.edit.total_amount_paid)" type="number" :placeholder="t('form-fields.total-amount-paid')" :label="t('form-fields.total-amount-paid')" errorField="total_amount_paid" :errors="props.validation" class="mb-3 d-none" />
      <InputControl disabled :modelValue="deCurrency(reserveStore.edit.to_pay)" type="number" :placeholder="t('form-fields.to-pay')" :label="t('form-fields.to-pay')" errorField="to_pay" :errors="props.validation" class="mb-3 d-none" />
      <FormSelect v-model="reserveStore.edit.payment_method" :options="paymentMethods" displayProperty="name" valueProperty="id" :label="t('form-fields.payment-method')" :placeholder="t('form-fields.payment-method')" errorField="payment_method" :frontError="props.validation" />
    </div>

    <div class="col-md-6 offset-xxl-2 col-xxl-5">
      <CurrencyControl v-model.number="reserveStore.edit.deposit" v-model:currency="reserveStore.editCurrency.deposit" :placeholder="t('form-fields.deposit')" :label="t('form-fields.deposit')" errorField="deposit" :errors="props.validation" class="mb-3" />
      <FormSelect v-model="reserveStore.edit.payment_method_for_deposit" :options="paymentMethods" displayProperty="name" valueProperty="id" class="mb-3" :label="t('form-fields.payment-method-for-deposit')" :placeholder="t('form-fields.payment-method-for-deposit')" errorField="payment_method_for_deposit" :frontError="props.validation" />
      <InputControl v-model="reserveStore.edit.preauthorization_code" type="number" :placeholder="t('form-fields.preauthorization-code')" :label="t('form-fields.preauthorization-code')" errorField="preauthorization_code" :errors="props.validation" class="mb-3" />
      <CurrencyControl v-model="reserveStore.edit.deductible_fire_theft" v-model:currency="reserveStore.editCurrency.deductible_fire_theft" :placeholder="t('form-fields.deductible-fire/theft')" :label="t('form-fields.deductible-fire/theft')" errorField="deductible_fire_theft" :errors="props.validation" class="mb-3" />
      <CurrencyControl v-model="reserveStore.edit.deductible_damage" v-model:currency="reserveStore.editCurrency.deductible_damage" :placeholder="t('form-fields.deductible-damage')" :label="t('form-fields.deductible-damage')" errorField="deductible_damage" :errors="props.validation" class="mb-3" />
      <CurrencyControl v-model="reserveStore.edit.deductible_liability" v-model:currency="reserveStore.editCurrency.deductible_liability" :placeholder="t('form-fields.deductible-liability')" :label="t('form-fields.deductible-liability')" errorField="deductible_liability" :errors="props.validation" />
