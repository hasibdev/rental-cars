...
const getRentalPeriod = (rent_days) => {
  ...
  <template v-if="!loading && data?.reservation && data?.customerinfo">
    <div class="row mx-0">
      <div class="col-md-6 col-xs-12">
        <div class="d-flex align-items-center mb-1">
          <BackButton /> <span class="add-fleet__title ms-3">{{t('dashboard-reservation.view-reservation')}}</span>
        </div>
      ...
      <div class="col-md-6 col-xs-12 text-right">
        ...
        <div class="d-flex align-items-center justify-content-end">
          <div class="text-18 fw-500 me-3" v-if="data?.reservation?.reservation_status== 'Rented' || data?.reservation?.reservation_status == 'Completed'">
            <app-link :path="`/reservierungen/vertrags-vorschau/${data?.reservation?.id}`" class="pointer">
              {{ t('button.open-contact') }}
            </app-link>
          </div> 
          <StatusButton :type="getStatus(data?.reservation?.reservation_status)" style="cursor: default;">
            <span v-if="data?.reservation?.reservation_status == 'Reserved' ">{{ t('form-fields.reserved')}}</span>
            <span v-if="data?.reservation?.reservation_status == 'Rented' ">{{ t('form-fields.rented')}}</span>
            <span v-if="data?.reservation?.reservation_status == 'Cancelled' ">{{ t('form-fields.cancelled')}}</span>
            <span v-if="data?.reservation?.reservation_status == 'Completed' ">{{ t('form-fields.completed')}}</span>
            <span v-if="data?.reservation?.reservation_status == 'Confirmed' ">{{ t('form-fields.confirmed')}}</span>
            <span v-if="data?.reservation?.reservation_status == 'Declined' ">{{ t('requests.declined')}}</span>
            <span v-if="data?.reservation?.reservation_status == 'Expired' ">{{ t('text.expired')}}</span>
          </StatusButton>
        </div>
      </div>
    </div>
          ...
          <div class="row mx-0 details-section">
            <div class="col-lg-6 col-md-12 col-sm-12 px-0">
              ...
              <div class="row mx-0 mb-2">
                ...
                <div class="col-lg-7 col-md-6 col-sm-12">
                  <p class="mb-0" v-if="locale == 'en'">{{moment(data?.reservation?.start_date).format('DD/MM/YYYY')}}
                  </p>
              ...
              <div class="row mx-0 mb-2">
                ...
                <div class="col-lg-7 col-md-6 col-sm-12">
                  <p class="mb-0" v-if="locale == 'en'">{{moment(data?.reservation?.start_time,
                  "HH:mm:ss").format('h:mmA')}}</p>
                  <p class="mb-0" v-else>{{moment(data?.reservation?.start_time, "HH:mm:ss").format('HH:mm')}}</p>
                </div>
              </div>
              <div class="row mx-0 mb-2">
                <div class="col-lg-5 col-md-6 col-sm-12">
                ...
                <div class="col-lg-7 col-md-6 col-sm-12">
                  <div v-if="data?.reservation?.actual_end_date">
                    <p class="mb-0" v-if="locale == 'en'">{{moment(data?.reservation?.actual_end_date).format('DD/MM/YYYY')}}</p>
                    <p class="mb-0" v-else>{{moment(data?.reservation?.actual_end_date).format('DD.MM.YYYY')}}</p>
                  </div>
                  <div v-else>
                    <p class="mb-0" v-if="locale == 'en'">{{moment(data?.reservation?.end_date).format('DD/MM/YYYY')}}</p>
                    <p class="mb-0" v-else>{{moment(data?.reservation?.end_date).format('DD.MM.YYYY')}}</p>
                  </div>
                </div>
              </div>
              <div class="row mx-0 mb-2">
                ...
                <div class="col-lg-7 col-md-6 col-sm-12">
                  <div v-if="data?.reservation?.actual_end_time">
                    <p class="mb-0" v-if="locale == 'en'">{{moment(data?.reservation?.actual_end_time,"HH:mm:ss").format('h:mmA')}}</p>
                    <p class="mb-0" v-else>{{moment(data?.reservation?.actual_end_time, "HH:mm:ss").format('HH:mm')}}</p>
                  </div>
                  <div v-else>
                    <p class="mb-0" v-if="locale == 'en'">{{moment(data?.reservation?.end_time,"HH:mm:ss").format('h:mmA')}}</p>
                    <p class="mb-0" v-else>{{moment(data?.reservation?.end_time, "HH:mm:ss").format('HH:mm')}}</p>
                  </div>
            ...
            <div class="col-lg-6 col-md-12 col-sm-12 px-0">
              ...
              <div class="row mx-0 mb-2">
                ...
                <div class="col-md-6 col-sm-12">
                  <p class="mb-0">{{moneyFormat(data?.reservation?.cost_per_extra_km)}}</p>
                </div>
              </div>
              <div class="row mx-0 mb-2">
                <div class="col-md-6 col-sm-12">
              ...
              <div class="row mx-0">
                ...
                <div class="col-lg-7 col-md-12 col-sm-12 px-0">
                  ...
                  <div class="row mx-0">
                    ...
                    <div class="col-md-6 col-sm-12">
                      <p class="details__item__value" v-if="data?.customerinfo?.date_of_issunce_of_driving_license">{{
                  ...
                  <div class="row mx-0">
                    ...
                    <div class="col-md-6 col-sm-12">
                      <p class="details__item__value" v-if="data?.customerinfo?.expiry_date_of_driving_license">{{
                ...
                <div class="row mx-0">
                  ...
                  <div class="col-lg-7 col-md-12 col-sm-12 px-0">
                    ...
                    <div class="row mx-0">
                      ...
                      <div class="col-md-6 col-sm-12">
                        <p class="details__item__value" v-if="driver.date_of_issue">{{
                        moment(driver.date_of_issue).format('DD.MM.YYYY') }}</p>
                      </div>
                    </div>
                    <div class="row mx-0">
                      <div class="col-md-6 col-sm-12 px-0">
                      ...
                      <div class="col-md-6 col-sm-12">
                        <p class="details__item__value" v-if="driver.due_date">{{
                        moment(driver.due_date).format('DD.MM.YYYY') }}</p>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        <div class="col-md-10 offset-md-1 col-sm-12">
          ...
          <div class="row mx-0 details-section">
            <div class="col-lg-4 col-md-12 col-sm-12">
              <div class="bordered-content py-3 px-4" style="border: 1px solid #dddddd; border-radius: 4px;width: 100%; min-height: 174px;">
                {{data?.reservation?.damages_content || t('FleetView.no-content') }}
              </div>
            </div>
            <div class="col-lg-8 col-md-12 col-sm-12 ">
              <div v-if="data?.reservation?.damages_art">
                <img class="img-fluid" :src="data?.reservation?.damages_art" alt="" style="background: #f0f0f0; padding-left: 2.5rem; padding-right: 2.5rem">
              </div>
              <div v-else>
          ...
          <div class="row mx-0 details-section">
            <div class="col-lg-6 col-md-12 col-sm-12 px-0">
              ...
              <div class="row mx-0 mb-2">
                ...
                <div class="col-lg-7 col-md-6 col-sm-12">
                  <!-- <p class="mb-0">{{parseInt(data?.reservation?.rental_hours / 24)}} {{(data?.reservation?.rental_hours / 24) > 1 ? t('form-fields.days') : t('form-fields.day') }}
                    {{data?.reservation?.rental_hours % 24}} {{t('form-fields.hours')}} </p> -->
                    <p class="mb-0">{{getRentalPeriod(data?.reservation?.rent_days)}} </p>
                </div>
              </div>
              <div class="row mx-0 mb-2">
                <div class="col-lg-5 col-md-6 col-sm-12">
    ...
    <div class="row mx-0">
      <div class="col-md-12 text-right">
        <PrimaryButton variant="light" style="min-width: 180px;" class="px-0">
          <app-link :path="`/reservierungen`" class="pointer w-100 h-100" style="color: #333; min-width: 180px;">
            {{t('button.back')}}
          </app-link>
        </PrimaryButton>
        <OutlineBgBtn style="background-color:white; color: #333; border-color: black; min-width: 180px;" class="mx-3">
          <app-link :path="`/reservierungen/aktualisieren/${data?.reservation?.id}`" class="pointer w-100 h-100" style="color: #333; width: 100%; height: 100%; ">
            {{t('button.edit')}}
          </app-link>
        </OutlineBgBtn>
        <PrimaryButton 
          ...
          v-if="data?.reservation?.end_date && data?.reservation?.end_time && data?.reservation?.start_date && data?.reservation?.start_time 
                && data?.reservation?.create_by && data?.reservation?.pick_up_loaction && data?.reservation?.return_location 
                && data?.reservation?.fuel_level_at_pick_up_id && data?.reservation?.km_included && data?.reservation?.cost_per_extra_km
                && data?.reservation?.km_at_pick_up && data?.reservation?.contact_language && data?.reservation?.deposit && data?.reservation?.payment_method"
        >
          <app-link :path="`/reservierungen/vertrags-vorschau/${data?.reservation?.id}`" class="pointer w-100 h-100 text-white">
            {{t('button.contract-preview')}}
          </app-link>
        </PrimaryButton>
        <PrimaryButton v-else variant="light" style="min-width: 180px; cursor: default;" class="text-muted" >
          {{t('button.contract-preview')}}
        </PrimaryButton>
      </div>
    </div>
