...
onMounted(() => {
  ...
  setTimeout(() => {
    reserveStore.form.damages_art = reserveStore.createInfo.artCopy
  }, 1000)
})

const filesPreview = ref([])
function removeHandoverImage(index) {
  reserveStore.form.photos_handover = reserveStore.form.photos_handover.filter((item, i) => i !== index)
}

watch(() => reserveStore.form.photos_handover, (newFiles) => {
...
<template>
  <div class="damages">
    <div class="damages__sketch-inner">
      <div class="row">
        <div class="col-lg-6">
          <TextControl v-model="reserveStore.form.damages_content" class="mb-5" :placeholder=" $t('form-fields.damage-description-at-handover')" :label="$t('form-fields.damages')" errorField="damages_content" />
          <div class="row">
            <div class="col-md-4">
              <h5 class="damages__label pt-2" style="color: #666666;">{{ $t('dashboard-reservation.photos-at-handover') }}</h5>
            </div>
            <div class="col-md-8">
              <DocumentsCameraUploader v-model="reserveStore.form.photos_handover" :showPreview="false"></DocumentsCameraUploader>
            </div>
          </div>
        </div>
        <div class="col-lg-4 offset-lg-2">
          <ArtBoard v-model="reserveStore.form.damages_art" />
        </div>
      </div>
