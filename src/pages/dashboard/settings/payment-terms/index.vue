...
} = useAxios('/admin/payment_terms')

const fields = [
  {
    field: 'paymentterms_name',
...
const onAddPaymentTerms = async () => {
  ...
  try {
    ...
    await api.post('/admin/payment_terms/store', formData)
    reFetch()
...
const onEditLocation = async () => {
  ...
  try {
    ...
    await api.post(`/admin/payment_terms/update/${editForm.data.id}`, formData)
    reFetch()
...
const onDelete = async () => {
  try {
    await onApiDelete(`/admin/payment_terms/delete/${activeDeleteId.value}`)
    reFetch()
...
<template>
  ...
  <div class="d-flex justify-content-between align-items-center">
    ...
    <button @click="addModalStatus = true" class="btn btn-dark">
      ...
      <span class="ms-3">+ {{t('paymentTermsSettings.add-payment-terms-text')}}</span>
    </button>
  </div>
  <h5 class="mt-4">{{t('locationSettings.title')}}</h5>

  <div class="borderd-content my-3">
    <DataTable bordered :data="paymentTermsData || []" :fields="fields" :loading="loading">
  ...
  <ModalControl v-model="editModalStatus" dialogClass="modal-lg modal-large">
    <div class="modal-header">
      <p class="mb-1">{{t('paymentTermsSettings.edit-payment-terms-text')}}</p>
    </div>
    <div v-if="editForm.data" class="modal-body">
      <InputControl v-model="editForm.data.paymentterms_name" :label="t('paymentTermsSettings.term-name')" placeholder="e.g. 20days" errorField="paymentterms_name" class="mt-3" />
      <InputControl v-model="editForm.data.days" :label="t('paymentTermsSettings.days')" errorField="days" placeholder="e.g. 15" class="mt-3" />
      <InputControl v-model="editForm.data.note" :label="t('paymentTermsSettings.note')" :placeholder="t('paymentTermsSettings.note')" class="mt-3" />
  ...
  <ModalControl v-model="addModalStatus" dialogClass="modal-lg">
    <div class="modal-header">
      <p class="mb-1">{{t('paymentTermsSettings.add-payment-terms-text')}}</p>
    </div>
    <div class="modal-body">
      <!-- <p class="fw-500">{{t('locationSettings.edit-modal-text')}}</p> -->

      <InputControl v-model="addForm.paymentterms_name" :label="t('paymentTermsSettings.term-name')" placeholder="e.g. 20 days" errorField="paymentterms_name" class="mt-3" />
      <InputControl v-model="addForm.days" :label="t('paymentTermsSettings.days')" placeholder="e.g. 15" class="mt-3" errorField="days" />
      <InputControl v-model="addForm.note" :label="t('paymentTermsSettings.note')" :placeholder="t('paymentTermsSettings.note')" class="mt-3" />

    </div>
    <div class="modal-footer border-0 mt-2">
      <button @click="addModalStatus = false" type="button" class="btn btn-cancel px-5 py-2 btn-sm ">{{t('button.cancel')}}</button>
      <PrimaryButton class="btn-save" @click="onAddPaymentTerms" :loading="addingState">{{t('button.save')}}</PrimaryButton>
    </div>
  </ModalControl>

  <!-- Delete Modal -->
  <ConfirmDialog v-model="showDeleteModal" @onOk="onDelete" :okLoading="deleting" :title="t('paymentTermsSettings.delete-payment-terms-title')" :message="t('paymentTermsSettings.delete-payment-terms-text')"></ConfirmDialog>
