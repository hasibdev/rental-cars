...
<template>
  <div class="mt-4">
    <DataTable :loading="loading" :data="fleetclassData || []" :fields="fields">
      <template #day_rate={row}>
        {{moneyFormat(row.day_rate, locale)}}
      </template>
      <template #week_rate={row}>
        {{moneyFormat(row.week_rate, locale)}}
      </template>
      <template #month_rate={row}>
        {{moneyFormat(row.month_rate, locale)}}
      </template>
  ...
  <!-- Entfernen Modal -->
  <ConfirmDialog v-model="showDeleteModal" @onOk="onDelete" :okLoading="deleting" :title="t('fleetSettings.delete-modal-title')" :message="t('fleetSettings.delete-modal-text')"></ConfirmDialog>
</template>
