...
const onAddFuel = async () => {
  ...
  try {
    ...
    await api.post('/admin/fuel/store', formData)
    reFetch()
...
const onEditFuel = async () => {
  ...
  try {
    ...
    await api.post(`/admin/fuel/update/${editForm.data.id}`, formData)
    reFetch()
...
const onDelete = async () => {
  try {
    await onApiDelete(`/admin/fuel/delete/${activeDeleteId.value}`)
    reFetch()
...
<template>
  ...
  <div class="d-flex justify-content-between align-items-center">
    ...
    <button @click="addModalStatus = true" class="btn btn-dark">
      ...
      <span class="ms-3">+ {{t('fuelSettings.add-fuel-text')}}</span>
    </button>
  </div>
  <h5 class="mt-4">{{t('fuelSettings.title')}}</h5>

  <div class="borderd-content my-3">
    <DataTable bordered :data="fuelData?.fules || []" :fields="fields" :loading="loading">
  ...
  <ModalControl v-model="addModalStatus" dialogClass="modal-md">
    <div class="modal-header">
      <p class="mb-1">{{t('fuelSettings.add-fuel-text')}}</p>
    </div>
    <div class="modal-body">
      <!-- <p class="fw-500">{{t('locationSettings.edit-modal-text')}}</p> -->

      <InputControl v-model="addForm.fuel_name" :label="t('fuelSettings.fuel-name')" placeholder="e.g. LPG" errorField="fuel_name" class="mt-3" />
    ...
    <div class="modal-footer border-0 mt-2">
      ...
      <PrimaryButton class="btn-save" @click="onAddFuel" :loading="addingState">{{t('button.save')}}</PrimaryButton>
    </div>
  </ModalControl>
  ...
  <ModalControl v-model="editModalStatus" dialogClass="modal-md">
    ...
    <div v-if="editForm.data" class="modal-body">
      ...
      <InputControl v-model="editForm.data.fuel_name" :label="t('fuelSettings.fuel-name')" placeholder="e.g. LPG" errorField="fuel_name" class="mt-3" />
  ...
  <ConfirmDialog v-model="showDeleteModal" @onOk="onDelete" :okLoading="deleting" title="Delete Fuel" :message="t('fuelSettings.delete-modal-text')"></ConfirmDialog>
