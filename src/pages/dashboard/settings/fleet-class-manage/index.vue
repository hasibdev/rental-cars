...
const fields = [
  ...
  {
    field: 'hour_rate',
    label: t('fleetSettings.hour-rate'),
  },
  // {
  //     field: 'updated_at',
  //     label: t('fleetSettings.updated'),
  //     handler: (row) =>{
...
const onAddFleet = async () => {
  ...
  try {
    ...
    await api.post('/admin/fleetclass/store', formData)
    reFetch()
...
const onEditFleet = async () => {
  ...
  try {
    ...
    await api.post(`/admin/fleetclass/update/${editForm.data.id}`, formData)
    reFetch()
...
const onDelete = async () => {
  try {
    await onApiDelete(`/admin/fleetclass/delete/${activeDeleteId.value}`)
    reFetch()
...
<template>
  ...
  <div class="d-flex justify-content-between align-items-center">
    ...
    <button @click="addModalStatus = true" class="btn btn-dark">
      ...
      <span class="ms-3">+ {{t('fleetSettings.add-fleet-text')}}</span>
    </button>
  </div>
  <h5 class="mt-4">{{t('fleetSettings.title')}}</h5>

  <div class="borderd-content my-3">
    <DataTable bordered :data="fleetClassData || []" :fields="fields" :loading="loading">
  ...
  <ModalControl v-model="addModalStatus" dialogClass="modal-lg">
    <div class="modal-header">
      <p class="mb-1">{{t('fleetSettings.add-fleet-text')}}</p>
    </div>
    <div class="modal-body">
      <!-- <p class="fw-500">{{t('locationSettings.edit-modal-text')}}</p> -->

      <InputControl v-model="addForm.classname" :label="t('fleetSettings.class-name')" placeholder="e.g. Kleinwagen" errorField="classname" class="mt-3" />
      <InputControl v-model="addForm.day_rate" :label="t('fleetSettings.day-rate-in-EUR')" placeholder="e.g. 500" errorField="day_rate" class="mt-3" />
      <InputControl v-model="addForm.hour_rate" :label="t('fleetSettings.hour-rate-in-EUR')" placeholder="e.g. 500" errorField="hour_rate" class="mt-3" />
      <InputControl v-model="addForm.note" :label="t('fleetSettings.note')" :placeholder="t('fleetSettings.note')" errorField="note" class="mt-3" />
    ...
    <div class="modal-footer border-0 mt-2">
      ...
      <PrimaryButton class="btn-save" @click="onAddFleet" :loading="addingState">{{t('button.save')}}</PrimaryButton>
    </div>
  </ModalControl>
  ...
  <ModalControl v-model="editModalStatus" dialogClass="modal-lg modal-large">
    <div class="modal-header">
      <p class="mb-1">{{t('fleetSettings.edit-fleet')}}</p>
    </div>
    <div v-if="editForm.data" class="modal-body">
      <InputControl v-model="editForm.data.classname" :label="t('fleetSettings.class-name')" placeholder="e.g. Kleinwagen" errorField="classname" class="mt-3" />
      <InputControl v-model="editForm.data.day_rate" :label="t('fleetSettings.day-rate-in-EUR')" placeholder="e.g. 500" errorField="day_rate" class="mt-3" />
      <InputControl v-model="editForm.data.hour_rate" :label="t('fleetSettings.hour-rate-in-EUR')" placeholder="e.g. 500" errorField="hour_rate" class="mt-3" />
      <InputControl v-model="editForm.data.note" :label="t('fleetSettings.note')" :placeholder="t('fleetSettings.note')" errorField="note" class="mt-3" />
    ...
    <div class="modal-footer border-0 mt-2">
      ...
      <PrimaryButton @click="onEditFleet" :loading="editingState">{{t('button.save')}}</PrimaryButton>
    </div>
  </ModalControl>
  ...
  <ConfirmDialog v-model="showDeleteModal" @onOk="onDelete" :okLoading="deleting" title="Delete Fleet Class" :message="t('fuelSettings.delete-modal-text')"></ConfirmDialog>
